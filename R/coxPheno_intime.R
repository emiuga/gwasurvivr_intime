coxPheno_intime <- function(pheno.file, 
                     covariates, 
                     id.column, 
                     inter.term, 
                     time.start,
    ...
    if(!is.null(covariates)){
        ...
        ### drop NAs
        pheno.file <- pheno.file[,c(id.column, time.start, time.stop, event, ok.covs)]
        pheno.file <- pheno.file[complete.cases(pheno.file),]
        ids <- pheno.file[[id.column]]
        
        ## covariates should be numeric!
        pheno.file <- as.matrix(pheno.file[,c(time.start, time.stop, event, ok.covs)])
        ...
        ### drop NAs
        pheno.file <- pheno.file[,c(id.column, time.start, time.stop, event)]
        pheno.file <- pheno.file[complete.cases(pheno.file),]
        ids <- pheno.file[[id.column]]
        
        ## time-event should be numeric!
        pheno.file <- as.matrix(pheno.file[,c(time.start, time.stop, event)])
    ...
    cox.params <- coxParam_intime(pheno.file,
                           time.start,
                           time.stop,
                           event, 
                           covariates, 
